...
Since the application relies on a stateless REST api, the clients will save a token on their local storage and
send it as an HTTP header (`x-auth-token`).

If you want to use a real redis, you can run it with the `redis` profile.
...
If you choose to run the application with neither of those profile, the application will fallback to classical http
sessions.
